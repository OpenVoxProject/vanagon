...
class Vanagon
  class Platform
    ...
    def is_rpm?
      return !!@name.match(/^(aix|amazon|cisco-wrlinux|el|eos|fedora|sles|redhat|redhatfips)-.*$/)
    end
    ...
    # Utility matcher to determine if the platform is a macos variety
    #
    # @return [true, false] true if it is a macos variety, false otherwise
    def is_macos?
      !!(@name =~ /^macos-.*$/)
    end
    ...
    def is_unix?
      return !!@name.match(/^(solaris|aix|macos)-.*$/)
    end
    ...
    def generate_compiled_archive(project)
      ...
      [
        ...
        "cd #{archive_directory}/#{name_and_version}; #{bill_of_materials_command}; #{tar} #{tar_flags} -cf ../../#{name_and_version_and_platform}.tar *",
