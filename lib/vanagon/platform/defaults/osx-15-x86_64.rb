platform "osx-15-x86_64" do |plat|
  ...
  plat.provision_with %Q(sudo su test -c 'echo | /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"') unless File.directory?('/usr/local/var/homebrew')
  plat.provision_with "sudo chown -R test /usr/local"
